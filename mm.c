...
/* MAKING MANUALLY */
#define WSIZE 4 /*word 혹은 header/ footer 사이즈 (바이트)*/
#define DSIZE 8 /* Double word 사이즈 (바이트 )*/
#define CHUNKSIZE (1<<12) /*1을 왼쪽으로 12번 이동한 것*/
#define MAX(x,y) ((x)>(y)? (x):(y))
#define PACK(size, alloc) ((size) | (alloc))
#define GET(p) (*(unsigned int*)(p)) // size나 이런 세팅하기 위해서 일부러 이렇게 해주는 이유? 
...
//make explicit 매크로 void 포인터로 설정할 수 있음! 왜냐면 null인지 주소값인지 확인할 수 없음
...
int mm_init(void)
{
    ...
    /* padding -- prologue header(size,1 )-- root(next) -- root(prev) -- prologue footer  */
    if ( (heap_listp = mem_sbrk(6*WSIZE)) == (void *)-1 )
        return -1;
    PUT(heap_listp, 0);/* alignment padding: 패딩 첫번째 패딩을 집어넣는다*/
    PUT(heap_listp + (1*WSIZE), PACK(DSIZE*2, 1)); /* data size를 16과 allocated */
    PUT(heap_listp + (2*WSIZE), NULL); /*root successor*/
    PUT(heap_listp + (3*WSIZE), NULL); /*root previouser*/
    PUT(heap_listp + (4*WSIZE), PACK(DSIZE*2, 1)); /* data size를 16과 allocated */
    PUT(heap_listp + (5*WSIZE), PACK(0, 1));  /*에필로그 헤더를 집어 넣기*/
...
static void *coalesce(void *bp)
{
    ...
    // case1. PREV와 NEXT 둘다 allocated 되어 있을 때! 
    if (prev_alloc && next_alloc){
        putFreeBlock(bp);
        return bp;
    }
    // case2. PREV는 alloc 되어있고 next는 alloc이 안되어있어! 
    ...
    // case3. PREV alloc은 안되어 있고 next가 alloc 되어 있어! 
    else if (!prev_alloc && next_alloc)
    {
    ...
    // case4. PREV 랑 NEXT alloc 둘다 되어있지 않을 때
    else 
    {
        removeBlock(PREV_BLKP(bp));
        removeBlock(NEXT_BLKP(bp));
